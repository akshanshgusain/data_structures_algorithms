...
vector<int> xDirectionVector = {2, 1, -1, -2, -2, -1, 1, 2};
vector<int> yDirectionVector = {1, 2, 2, 1, -1, -2, -2, -1};
...
bool solveKTUtil(
    ...
    for (int k = 0; k < 8; k++) {
        next_x = x + xDirectionVector[k];
        next_y = y + yDirectionVector[k];

        if (isSafe(next_x, next_y, chessBoard)) {
            chessBoard[next_x][next_y] = moveCounter;

            if (solveKTUtil(next_x, next_y, moveCounter + 1, chessBoard, n)) {
                return true;
            } else {
                // backtrack
...
void solveKT() {
    ...
    if (!solveKTUtil(0, 0, 1, chessBoard, chessBoard.size())) {
